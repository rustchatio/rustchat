...
const roleColors: Record<string, string> = {
    ...
    team_admin: 'bg-brand/10 text-brand',
    member: 'bg-bg-surface-2 text-text-2',
...
<template>
    <div class="space-y-6" @click="closeMenu">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-text-1">User Management</h1>
                <p class="text-text-3 mt-1">Manage users, roles, and permissions</p>
            </div>
            <button 
                @click.stop="showCreateModal = true"
                class="flex items-center px-4 py-2 bg-brand hover:bg-brand/90 text-white rounded-lg font-medium transition-colors"
            >
        ...
        <div class="flex items-center space-x-4">
            <div class="relative flex-1 max-w-md">
                <Search class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-text-4" />
                <input 
                    v-model="searchQuery"
                    type="text"
                    placeholder="Search by name or email..."
                    class="w-full pl-10 pr-4 py-2 border border-border-2 rounded-lg bg-bg-surface-1 text-text-1 focus:ring-2 focus:ring-brand focus:border-transparent"
                />
            </div>
            <select 
                v-model="statusFilter"
                class="px-4 py-2 border border-border-2 rounded-lg bg-bg-surface-1 text-text-1"
            >
            ...
            <label class="inline-flex items-center gap-2 text-sm text-text-2">
                <input v-model="includeDeleted" type="checkbox" class="w-4 h-4 text-brand rounded" />
                Show deleted
            </label>
        </div>

        <!-- Users Table -->
        <div class="bg-bg-surface-1 rounded-xl shadow-sm border border-border-1 overflow-hidden">
            <table class="min-w-full divide-y divide-border-1">
                <thead class="bg-bg-surface-2">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-text-3 uppercase tracking-wider">User</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-text-3 uppercase tracking-wider">Role</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-text-3 uppercase tracking-wider">Status</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-text-3 uppercase tracking-wider">Last Login</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-text-3 uppercase tracking-wider">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-border-1">
                    ...
                    >
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-text-1 flex items-center gap-2">
                                        {{ user.display_name || user.username }}
                                        <span v-if="isDeleted(user)" class="px-2 py-0.5 text-xs rounded bg-danger/10 text-danger">
                                            Deleted
                                        </span>
                                    </div>
                                    <div class="text-sm text-text-3">{{ user.email }}</div>
                                    <div v-if="isDeleted(user) && user.delete_reason" class="text-xs text-danger">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span v-if="isDeleted(user)" class="flex items-center text-danger">
                                <Trash2 class="w-4 h-4 mr-1" /> Deleted
                            </span>
                            <span v-else-if="user.is_active" class="flex items-center text-success">
                                <UserCheck class="w-4 h-4 mr-1" /> Active
                            </span>
                            <span v-else class="flex items-center text-text-3">
                                <UserX class="w-4 h-4 mr-1" /> Inactive
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-text-3">
                            {{ formatDate(user.last_login_at) }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium relative">
                            <button 
                                @click.stop="handleEdit(user)"
                                class="text-brand hover:text-brand/80 mr-3"
                                title="Edit User"
                            >
                                <Edit2 class="w-4 h-4" />
                            </button>
                            <div class="relative inline-block text-left">
                                <button 
                                    ...
                                    class="text-text-4 hover:text-text-3"
                                >
                                ...
                                <div 
                                    ...
                                    class="absolute right-0 mt-2 w-48 bg-bg-surface-1 rounded-md shadow-lg py-1 z-10 border border-border-1 ring-1 ring-text-1/5"
                                >
                                    <button
                                        v-if="user.is_active && !isDeleted(user)"
                                        @click.stop="handleDeactivate(user)"
                                        class="flex w-full items-center px-4 py-2 text-sm text-danger hover:bg-bg-surface-2"
                                    >
                                    ...
                                    <button
                                        ...
                                        class="flex w-full items-center px-4 py-2 text-sm text-success hover:bg-bg-surface-2"
                                    >
                                    ...
                                    <button
                                        ...
                                        class="flex w-full items-center px-4 py-2 text-sm text-warning hover:bg-bg-surface-2"
                                    >
                                    ...
                                    <button
                                        ...
                                        class="flex w-full items-center px-4 py-2 text-sm text-brand hover:bg-bg-surface-2"
                                    >
                    ...
                    <tr v-if="adminStore.users.length === 0 && !adminStore.loading">
                        <td colspan="5" class="px-6 py-12 text-center text-text-3">
                            <Users class="w-12 h-12 mx-auto mb-4 text-text-4" />
            ...
            <div class="w-full max-w-lg rounded-xl border border-border-1 bg-bg-surface-1 shadow-xl">
                <div class="flex items-start justify-between p-5 border-b border-border-1">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <h3 class="text-lg font-semibold text-text-1">Delete User</h3>
                            <p class="text-sm text-text-3 mt-1">
                                This soft-deletes the account, revokes access, and hides it from normal user lists.
                            </p>
                        </div>
                    </div>
                    <button @click="closeDeleteModal" class="text-text-4 hover:text-text-2">
                        <X class="w-5 h-5" />
                    </button>
                </div>
                ...
                <div class="p-5 space-y-4">
                    ...
                    <div class="text-sm text-text-2">
                        ...
                        <div class="mt-2 rounded-md bg-bg-surface-2 px-3 py-2 font-mono text-xs break-all">
                            {{ deletingUser.username }} or {{ deletingUser.email }}
                        </div>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-text-2 mb-1">Confirmation text</label>
                        <input
                    ...
                    <div>
                        <label class="block text-sm font-medium text-text-2 mb-1">Reason (optional)</label>
                        <textarea
                    ...
                    <div v-if="deleteError" class="rounded-lg border border-danger/20 bg-danger/10 px-3 py-2 text-sm text-danger">
                        {{ deleteError }}
                    </div>
                </div>

                <div class="flex items-center justify-end gap-3 p-5 border-t border-border-1">
                    <button
                        @click="closeDeleteModal"
                        class="px-4 py-2 rounded-lg border border-border-2 text-text-2"
                    >
                        Cancel
                    </button>
                    <button
                        ...
                        class="px-4 py-2 rounded-lg bg-danger hover:bg-danger/90 disabled:opacity-50 text-white font-medium"
                    >
            ...
            <div class="w-full max-w-lg rounded-xl border border-border-1 bg-bg-surface-1 shadow-xl">
                <div class="flex items-start justify-between p-5 border-b border-border-1">
                    <div class="flex items-start gap-3">
                        <div class="rounded-full bg-warning/10 p-2">
                            <Eraser class="w-5 h-5 text-warning" />
                        </div>
                        <div>
                            <h3 class="text-lg font-semibold text-text-1">Wipe User</h3>
                            <p class="text-sm text-text-3 mt-1">
                                Permanently delete this user from the database. This action cannot be undone.
                            </p>
                        </div>
                    </div>
                    <button @click="closeWipeModal" class="text-text-4 hover:text-text-2">
                        <X class="w-5 h-5" />
                    </button>
                </div>

                <div class="p-5 space-y-4">
                    <div class="rounded-lg border border-warning/20 bg-warning/10 p-3 text-sm text-warning">
                    ...
                    <div class="text-sm text-text-2">
                        ...
                        <p class="mt-2 text-xs text-text-3">Deleted at: {{ formatDate(wipingUser.deleted_at ?? null) }}</p>
                    </div>

                    <div v-if="wipeError" class="rounded-lg border border-danger/20 bg-danger/10 px-3 py-2 text-sm text-danger">
                        {{ wipeError }}
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 p-5 border-t border-border-1">
                    ...
                    <button
                        ...
                        class="px-4 py-2 rounded-lg bg-warning hover:bg-warning/90 disabled:opacity-50 text-white font-medium"
                    >
        ...
        >
            <div class="w-full max-w-lg rounded-xl border border-border-1 bg-bg-surface-1 shadow-xl">
                <div class="flex items-start justify-between p-5 border-b border-border-1">
                    <div class="flex items-start gap-3">
                        <div class="rounded-full bg-brand/10 p-2">
                            <UserPlus class="w-5 h-5 text-brand" />
                        </div>
                        <div>
                            <h3 class="text-lg font-semibold text-text-1">Re-sync Memberships</h3>
                            <p class="text-sm text-text-3 mt-1">
                                Apply auto-membership policies for this user across all their teams.
                            </p>
                        </div>
                    </div>
                    <button @click="closeResyncModal" class="text-text-4 hover:text-text-2">
                        <X class="w-5 h-5" />
                    </button>
                </div>
                ...
                <div class="p-5 space-y-4">
                    ...
                    <div class="text-sm text-text-2">
                        <p>User:</p>
                        <div class="mt-2 rounded-md bg-bg-surface-2 px-3 py-2 font-medium">
                    ...
                    <div v-if="resyncResult" class="rounded-lg border border-success/20 bg-success/10 p-4">
                        <h4 class="text-sm font-semibold text-success mb-2">Re-sync Complete</h4>
                        <div class="grid grid-cols-3 gap-4 text-center">
                            <div>
                                <div class="text-2xl font-bold text-success">{{ resyncResult.teams_processed }}</div>
                                <div class="text-xs text-success/80">Teams Processed</div>
                            </div>
                            <div>
                                <div class="text-2xl font-bold text-success">{{ resyncResult.memberships_applied }}</div>
                                <div class="text-xs text-success/80">Applied</div>
                            </div>
                            <div>
                                <div class="text-2xl font-bold" :class="resyncResult.memberships_failed > 0 ? 'text-danger' : 'text-success'">{{ resyncResult.memberships_failed }}</div>
                                <div class="text-xs" :class="resyncResult.memberships_failed > 0 ? 'text-danger' : 'text-success'">Failed</div>
                            </div>
                        </div>
                    </div>

                    <div v-if="resyncError" class="rounded-lg border border-danger/20 bg-danger/10 px-3 py-2 text-sm text-danger">
                        {{ resyncError }}
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 p-5 border-t border-border-1">
                    ...
                    <button
                        v-if="resyncResult"
                        @click="closeResyncModal"
                        class="px-4 py-2 rounded-lg bg-text-3 hover:bg-text-2 text-white font-medium"
                    >
                        Close
                    </button>
                    <button
                        ...
                        class="px-4 py-2 rounded-lg bg-brand hover:bg-brand/90 disabled:opacity-50 text-white font-medium flex items-center"
                    >
