...
fn test_deny_toml_exists() {
    ...
    assert!(deny_toml.contains("[advisories]"), "deny.toml should contain advisories section");
    assert!(deny_toml.contains("[licenses]"), "deny.toml should contain licenses section");
    assert!(deny_toml.contains("[bans]"), "deny.toml should contain bans section");
    ...
    assert!(deny_toml.contains("BSD-2-Clause"), "Should allow BSD-2-Clause");
    assert!(deny_toml.contains("BSD-3-Clause"), "Should allow BSD-3-Clause");
}
...
fn test_security_headers_exist() {
    ...
    assert!(middleware_code.contains("X-Frame-Options"), "Should set X-Frame-Options");
    assert!(middleware_code.contains("X-Content-Type-Options"), "Should set X-Content-Type-Options");
    assert!(middleware_code.contains("X-XSS-Protection") || middleware_code.contains("Content-Security-Policy"), 
...
fn test_ci_security_checks() {
    ...
    assert!(ci_yaml.contains("security-audit"), "CI should have security-audit job");
    assert!(ci_yaml.contains("license-check"), "CI should have license-check job");
    
    // Verify security checks run before tests
    assert!(ci_yaml.contains("needs: [security-audit, license-check]"), 
...
fn test_docker_publish_security() {
    ...
    assert!(docker_yaml.contains("sign"), "Docker publish should sign images");
    
    // Verify SBOM generation is configured
    assert!(docker_yaml.contains("SBOM"), "Docker publish should generate SBOMs");
    
    // Verify vulnerability scanning
    assert!(docker_yaml.contains("Trivy"), "Docker publish should include Trivy scanning");
}
...
fn test_weak_algorithms_rejected() {
    ...
    for alg in sha1_algorithms {
        assert!(
            ...
            "Weak algorithm {} should not be allowed in digest algorithms", alg
        );
    }
    ...
    for alg in md5_algorithms {
        assert!(
            !config.allowed_signature_algorithms.contains(&alg.to_string()),
            "Weak algorithm {} should not be allowed in signature algorithms", alg
        );
    }
}
